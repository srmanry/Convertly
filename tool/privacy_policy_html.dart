...
String privacyPolicyHtml() {
  final StringBuffer out = StringBuffer()
    ...
    ..writeln('.date{color:#666}ul{padding-left:1.4rem}')
    ..writeln('footer{margin:2.5rem 0;text-align:center}')
    ..writeln('</style>')
    ..writeln('</head>')
    ..writeln('<body>')
    ...
    ..writeln(
      ...
      '${_e(PrivacyPolicy.lastUpdatedDate)}</p>',
    );

  for (final String paragraph in PrivacyPolicy.introParagraphs) {
  ...
  for (final PolicySection section in PrivacyPolicy.sections) {
    ...
    if (section.bullets.isNotEmpty) {
      ...
      for (final String bullet in section.bullets) {
        out.writeln('<li>${_link(bullet)}</li>');
      }
      out.writeln('</ul>');
    }
  }
  ...
  out
    ..writeln('<footer><strong>${_e(PrivacyPolicy.appName)}</strong><br>')
    ..writeln('${_e(PrivacyPolicy.footerTagline)}</footer>')
    ..writeln('</body>')
    ..writeln('</html>');
  return out.toString();
}
